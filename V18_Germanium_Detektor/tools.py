...
def pint_curve_fit(fit_fn, x, y, param_units, bounds=None, p0=None, return_p0=False):
    ...
    if p0 is not None:
        assert all(isinstance(p, pint.Quantity)
                   for p in p0), f"p0 requires all elements to be pint Quantities. Got {[type(p0_i) for p0_i in p0]}"
        assert not any(isinstance(p.m, UFloat) for p in p0), "p0 must not contain uncertainties"
        p0 = [
...
def plot_context(plt, xunits, yunits, xname=None, yname=None):
    ...
    class MyPlotter:
        ...
        def plot(self, *args, show_xerr=True, show_yerr=True, **kwargs):
            ...
            if (x_s is not None) or (y_s is not None):
                assert not more_args  # Sonst versucht errorbar, daraus ein zweites yerr zu lesen.
                return plt.errorbar(x_n, y_n, xerr=x_s, yerr=y_s, **kwargs)
            else:
    ...
    def fmt_label(name, units):
        if units == pint.Unit('dimensionless'):
            ...
            if BUILD:
                if units == pint.Unit('kiloelectron_volt'):
                    # TODO: Fix upstream or generalize to all eV units
                    return f"${name}" + r" \mathbin{/} " + r"\si[]{\kilo\electronvolt}$"
