...
# Fit berechnen
params = tools.pint_curve_fit(calc_I, ɑ, I, (ureg.deg, ureg.microwatt))
# params = tools.pint_curve_fit(calc_I, ɑ, I, (1*ureg.deg, 1*ureg.microwatt), p0=(90 * ureg.deg, max(I)))
# params = tools.pint_curve_fit(calc_I, ɑ, I, (ureg.deg, ureg.microwatt), p0=(ureg('90 °'), max(I)))
print(f"params: {params}")
nominal_params = [tools.nominal_value(p) for p in params]
print(f"nominal_params: {nominal_params}")
ɑ_linspace = tools.linspace(*tools.bounds(ɑ))


with tools.plot_context(plt, '°', 'microwatt', 'ɑ', 'I') as plt2:
